(ns fueltrade.operation
  "OperationActor -- one wholesale-fuel operation = one supervised actor
  run, expressed as a langgraph-clj StateGraph. The advisor
  (FuelTradeAdvisor) is sealed into a single node (:advise); its
  proposal is ALWAYS routed through the Fuel Trading Governor
  ...
  One graph run = one wholesale-fuel operation (intake -> advise ->
  govern -> decide -> commit | hold | approval). No unbounded inner
  loop -- each operation is auditable and checkpointed. A fuel-order's
  life is advanced by MANY ops (intake / verify / dispatch / settle),
  each its own independent run.

  Human-in-the-loop = real approval workflow:
  `interrupt-before #{:request-approval}` pauses the actor and hands the
  decision to a human operator (trading supervisor / rack operator).
  The approver resumes with `{:approval {:status :approved}}` (or
  :rejected). `:delivery/dispatch`/`:invoice/settle` ALWAYS reach this
  node when the governor is clean -- see `fueltrade.phase`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [fueltrade.fueltradeadvisor :as fueltradeadvisor]
            [fueltrade.governor :as governor]
...
(defn build
  ...
  `fueltrade.store/Store`).
  opts:
    :advisor      -- a `fueltrade.fueltradeadvisor/Advisor` (default: mock-advisor)
    :checkpointer -- langgraph checkpointer (default: in-mem)"
  [store & [{:keys [advisor checkpointer]
             :or   {advisor      (fueltradeadvisor/mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; FuelTradeAdvisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (fueltradeadvisor/-advise advisor store request)]
            {:proposal p :audit [(fueltradeadvisor/trace request p)]})))

      ;; Fuel Trading Governor -- independent censor (separate system than the LLM).
      (g/add-node :govern
